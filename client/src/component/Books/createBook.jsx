...
function CreateBook() {
  ...
  const onSubmit = (data) => {
    if (!bookFile) {
      return toast.error("يرجى اضافة الكتاب");
    }
    if (!coverImage) {
      return toast.error("يرجى اضافة صوره الكتاب");
    }
  ...
  return (
    <Container dir={i18n.dir()}>
      <h2 className="card-code-title">{t("addBookForm.addNewBook")}</h2>
          ...
          <Col lg={6} md={12}>
            ...
            <Form.Group controlId="formCoverImage" className="mb-4">
              ...
              >
                ...
                <p className="text-muted small">
                  {t("addBookForm.coverImagePlaceholder")}
                </p>
              </div>
              {errors.coverImage && (
